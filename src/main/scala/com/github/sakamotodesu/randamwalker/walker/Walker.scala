...
class NoPlanWalker(map: Map, maxTurn: Int, way: List[Point]) extends Walker(map, maxTurn, way) with RandomWalk{
  def think(w:List[Point],next:List[Point]) =  random(w,next)
  override def toString = "NoPlanWalker"
}

class StraightWalker(map: Map, maxTurn: Int, way: List[Point]) extends Walker(map, maxTurn, way) with StraightWalk{
  def think(w:List[Point],next:List[Point]) = straight(w,next)
  override def toString = "StraightWalker"
}

class StraightPrudentWalker(map: Map, maxTurn: Int, way: List[Point]) extends Walker(map, maxTurn, way) with RandomWalk with StraightWalk{
...
class Plan8020Walker(map: Map, maxTurn: Int, way: List[Point]) extends Walker(map, maxTurn, way) with RandomWalk with StraightWalk with Probability{
  override def think(w:List[Point],next:List[Point]) =
    if (toBeTrue(80)) straight(w,next) else random(w,next)
  override def toString = "Plan8020Walker"
}

class ProbWalker(prob: Int, map: Map, maxTurn: Int, way: List[Point]) extends Walker(map, maxTurn, way) with RandomWalk with StraightWalk with Probability{
  override def think(w:List[Point],next:List[Point]) =
